...
model = AutoModelForCausalLM.from_pretrained(main_folder_path+'/Model_FineTuned/RITA_FineTuning_runclm_256/checkpoint-BEST-MODEL', trust_remote_code=True)
tokenizer = AutoTokenizer.from_pretrained(main_folder_path+'/Model_FineTuned/RITA_FineTuning_runclm_256/checkpoint-BEST-MODEL')
...
for i in range(len(unique_start)):
    ...
    csv_filename = main_folder_path + '/Generation_Sequences/run_clm_256/' + 'generated_sequences_' + str([unique_start[i]]) + '.csv'
