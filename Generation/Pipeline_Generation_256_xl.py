...
model = AutoModelForCausalLM.from_pretrained(main_folder_path+'/Model_FineTuned/RITA_FineTuning_runclm_256_xl/checkpoint-100130', trust_remote_code=True)
tokenizer = AutoTokenizer.from_pretrained(main_folder_path+'/Model_FineTuned/RITA_FineTuning_runclm_256_xl/checkpoint-100130')
...
for i in range(len(unique_start)):
    ...
    csv_filename = main_folder_path + '/Generation_Sequences/run_clm_256_xl/' + 'generated_sequences_' + str([unique_start[i]]) + '.csv'
