...
tokenizer = AutoTokenizer.from_pretrained("rahuldhodapkar/protgpt2-finetuned-sarscov2-rbd")
model = AutoModelForCausalLM.from_pretrained("rahuldhodapkar/protgpt2-finetuned-sarscov2-rbd")
...
for i in range(len(unique_start)):
    ...
    csv_filename = main_folder_path + '/Generation_Sequences/SpikeGTP/' + 'generated_sequences_' + str([unique_start[i]]) + '.csv'
